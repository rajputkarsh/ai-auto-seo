...
- ✅ **Repo-rail orchestration** (`applyRepoRail`) — runs the rail across every finding in a scan; each finding independently opens a PR or falls back, so one decline never blocks the others. Opened PRs are recorded for tracking.
- ✅ **Merge-rate outcome tracker** (`PrOutcomeStore`, §10) — records opened PRs, resolves them to `merged_unedited | merged_edited | dismissed`, and computes **merge-rate-without-edits over *resolved* PRs** (an open PR is excluded from the ratio; a merged-but-edited PR counts as a miss). This is the Phase-3 exit-gate metric, now measurable.
