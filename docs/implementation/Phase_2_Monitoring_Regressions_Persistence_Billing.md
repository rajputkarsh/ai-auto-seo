...
- ✅ **Billing, entitlements & usage metering** (`@awe/billing`). A tier catalog (free/pro/team/enterprise), per-org subscriptions with overrides, a monthly usage meter, and a `QuotaGuard` — all pure and fully tested. Wired into the API: scans resolve an org (see **Authentication** below), are gated on quota (**402** when suspended or over limit), have their page budget clamped to the plan, and record usage — including LLM cost captured via the reasoner's event hook — only *after* success, so a failed or rejected scan is never billed. `GET /billing/status` exposes it. Stripe is behind a `BillingProvider` seam (manual/Free-only default); the enforcement logic needs no Stripe account.
- ✅ **Authentication** (`@awe/auth`) — real, replacing the earlier `x-awe-org` stand-in. Org-scoped **API keys**: the plaintext is shown once and only its SHA-256 is stored, so a datastore leak exposes no usable credential; a presented key is hashed and looked up, and resolves to an `Identity { orgId, role }`. A single API `onRequest` hook attaches identity to every non-public request and returns **401** when it can't. Two modes: `apikey` (production — only a valid `Authorization: Bearer awe_…` authenticates; the `x-awe-org` header grants nothing) and `dev` (local zero-config — trusts `x-awe-org` as an owner when no key is presented). Default is `apikey` in production, `dev` otherwise. Keys are minted/listed/revoked by the superadmin (`/admin/orgs/:orgId/keys`), and the customer dashboard has a real **sign-in** (API key → httpOnly cookie → Bearer). The store is in-memory by default, Postgres-backed under `DATABASE_URL`, proven identical by a shared contract test.
